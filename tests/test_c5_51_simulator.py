...
from mine_env.simulator_c5_51 import run_policy_simulation
...
def test_one_seed_smoke_completes(config):
    ...
    route_total = sum(summary[f"route_r_{key}_loads"] for key in ("a1", "a2", "b1", "b2", "c1", "c2"))
    assert route_total == summary["completed_loads"]
...
def test_all_c5_51_policies_smoke(config, policy_id):
