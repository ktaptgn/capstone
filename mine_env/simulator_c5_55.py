...
from mine_env.policies_c5_55 import GUARD_METRIC_KEYS, create_h5_policy
...
def run_policy_simulation(
    ...
    """Run one C5.55 official H5 benchmark policy through the C5.53 simulator."""
    policy = create_h5_policy(policy_id, config)
...
def write_summary(summary_rows: list[dict[str, Any]], summary_dir: str | Path) -> None:
    ...
    csv_path = target_dir / "c5_55_policy_comparison.csv"
    json_path = target_dir / "c5_55_policy_comparison.json"
...
def write_daily_records(daily_rows: list[dict[str, Any]], log_dir: str | Path) -> Path:
    ...
    target_dir.mkdir(parents=True, exist_ok=True)
    csv_path = target_dir / "c5_55_daily_summary.csv"
    with csv_path.open("w", encoding="utf-8", newline="") as stream:
...
def write_dispatch_events(event_rows: list[dict[str, Any]], log_dir: str | Path) -> Path:
    target_dir = Path(log_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    csv_path = target_dir / "c5_55_dispatch_events.csv"
    with csv_path.open("w", encoding="utf-8", newline="") as stream:
