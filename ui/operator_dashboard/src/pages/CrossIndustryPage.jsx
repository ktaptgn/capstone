import { Plane, Wind, Atom, Ship, Trees, ArrowRight, Lightbulb } from 'lucide-react';
...
export default function CrossIndustryPage() {
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      {/* Framing */}
      <Card style={{ padding: 24, borderLeft: '4px solid #8A4931' }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          <div style={{
            width: 44, height: 44, borderRadius: 10, background: 'var(--primary-soft)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}>
            <Lightbulb size={22} color="#8A4931" />
          </div>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--text-main)' }}>
              핵심은 광산이 아니라 “열화 인지 기반 운영 의사결정”
            </h2>
            <p style={{ margin: '8px 0 0', fontSize: 13, color: 'var(--text-body)', lineHeight: 1.6 }}>
              본 프레임워크의 본질은 <strong>수요와 정비 용량 제약 하에서 자산 열화를 반영해
              “언제·무엇을·어떻게 정비할지”를 결정</strong>하는 것입니다. 광산 트럭은 하나의 대리(proxy)
              도메인일 뿐이며, 동일한 의사결정 구조는 안전·가동률이 중요한 여러 산업으로 전이됩니다.
            </p>
            <div style={{
              marginTop: 12, fontSize: 11, color: '#92400E', background: '#FFFBEB',
              border: '1px solid #FDE68A', borderRadius: 8, padding: '8px 12px',
            }}>
              ※ 본 시스템은 실제 Escondida 디지털 트윈이 아니라, 열화 인지 의사결정을 보여주는
              <strong> proxy DES 의사결정 지원 시스템</strong>입니다.
            </div>
          </div>
        </div>
      </Card>
