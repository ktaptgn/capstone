"""Run the C5.54 guarded H4 route-allocation experiment."""
...
from mine_env.simulator_c5_54 import (
...
SUMMARY_DIR = PROJECT_ROOT / "outputs" / "c5_54" / "summary"
LOG_DIR = PROJECT_ROOT / "outputs" / "c5_54" / "logs"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="C5.54 guarded H4 route-allocation sweep.")
    ...
    for regime in regimes:
        ...
        for sensitivity in sensitivities:
            print(
                f"=== C5.54 {regime} sensitivity={sensitivity}: "
    ...
    print(f"\nWrote {Path(args.summary_dir) / 'c5_54_policy_comparison.csv'}", flush=True)
    if not args.no_event_log:
