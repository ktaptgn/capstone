"""Run the C5.5 experimental facility-destination policy sweep."""
...
from mine_env.config_c5_5 import CONFIG_PATH, load_config
from mine_env.simulator_c5_5 import run_policy_simulation, write_summary

SUMMARY_DIR = PROJECT_ROOT / "outputs" / "c5_5" / "summary"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="C5.5 facility-destination PM+dispatch sweep.")
    ...
    for regime in regimes:
        ...
        print(
            f"=== C5.5 {regime}: {len(policies)} policies x {len(seeds)} seeds x {eff_days} days ===",
            flush=True,
        )
    ...
    print(f"\nWrote {Path(args.summary_dir) / 'c5_5_policy_comparison.csv'}", flush=True)
    return 0
