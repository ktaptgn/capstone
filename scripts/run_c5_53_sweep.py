"""Run the C5.53 congestion-aware route-facility sweep."""
...
SUMMARY_DIR = PROJECT_ROOT / "outputs" / "c5_53" / "summary"
LOG_DIR = PROJECT_ROOT / "outputs" / "c5_53" / "logs"
ANALYSIS_DIR = PROJECT_ROOT / "outputs" / "c5_53" / "analysis"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="C5.53 congestion-aware route-facility sweep.")
    ...
    seeds = (
        ...
        if args.seeds
        else [int(s) for s in base_cfg["evaluation"]["seeds"][:10]]
    )
    ...
    for regime in regimes:
        ...
        for sensitivity in sensitivities:
            print(
                f"=== C5.53 {regime} sensitivity={sensitivity}: "
            ...
            for policy_id in policies:
                ...
                print(
                    ...
                    f"TCOv3={mean(rows, 'total_tco_v3'):.1f} "
                    f"congH={mean(rows, 'congestion_delay_hours_total'):.1f} "
    ...
    print(f"\nWrote {Path(args.summary_dir) / 'c5_53_policy_comparison.csv'}", flush=True)
    if not args.no_event_log:
