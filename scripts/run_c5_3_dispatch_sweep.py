"""C5.3 dispatch-only sweep: H0-H4 recast as route-selection strategies, shared fixed
rule-based PM, 3-component reliability (frailty + sensor noise) in the 25-truck C5 mine.

Runs every enabled dispatch policy across held-out seeds and one or more operating regimes,
then writes a per-regime ranked KPI table, a per-seed robustness matrix (paired vs the H0
route-blind baseline), and a cross-regime summary that flags where the failure/CM path
activates and whether the ranking flips. Lower total_tco is better. In-lab benchmark.
"""
...
from mine_env.simulator_c5_3 import run_policy_simulation, write_summary

CONFIG_PATH = PROJECT_ROOT / "configs" / "c5_3.yaml"
REPORT_PATH = PROJECT_ROOT / "docs" / "source" / "06_C5_3_RESULTS.md"
SUMMARY_DIR = PROJECT_ROOT / "outputs" / "c5_3" / "summary"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="C5.3 dispatch-only sweep.")
    ...
    header = [
        ...
        "frailty (Gamma wear heterogeneity) and C6 sensor noise. Decision target = DISPATCH only; "
        "PM delegated to a FIXED CBM rule engine (H*=0.21/0.21/0.28) shared byte-identically by "
        "every policy. Objective = total TCO (pm+cm+downtime+degradation+unmet).",
        "- Policies (route-selection recast of the C5.2 heuristics): H0 route-blind baseline, "
        "H1 health routing, H2 risk-aware routing, H3 value (grade) routing, H4 capacity routing.",
    ...
    for regime in regimes:
        ...
        for pid, rows in results.items():
            ...
            print(f"  {pid}: TCO={mean(rows,'total_tco'):.1f} CM={mean(rows,'cm_count'):.1f} "
                  f"fail={mean(rows,'failure_count'):.1f}", flush=True)
    ...
    print(f"\nWrote {args.report}\nWrote {SUMMARY_DIR / 'c5_3_policy_comparison.csv'}", flush=True)
    return 0
